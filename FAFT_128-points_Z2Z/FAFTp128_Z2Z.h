...
inline __device__ void load128_half_Z2Z_ax0( int radix, double2 *z, double2 *a, int sx )
{
    for( int i = 0; i < radix; i++ )
        if ( i < (radix>>1) )
            z[i] = a[(i*sx)<<6];
...
inline __device__ void load128_half_Z2Z_ax1( int radix, double2 *z, double2 *a, int sx )
{
    for( int i = 0; i < radix; i++ )
        if ( i < (radix>>1) )
            z[i] = a[(i*sx)];
...
inline __device__ void load128_half_Z2Z_ax2( int radix, double2 *z, double2 *a, int sx )
{
    for( int i = 0; i < radix; i++ )
        if ( i < (radix>>1) )
            z[i] = a[(i*sx)<<12];
...
inline __device__ void load128_half_Z2Z_ax3( int radix, double2 *z, double2 *a, int sx )
{
    for( int i = 0; i < radix; i++ )
        if ( i < (radix>>1) )
            z[i] = a[(i*sx)<<18];
...
template<int radix> inline __device__ void store128_half_Z2Z_ax0( double2 *z, double2 *a, int sx )
{
    #pragma unroll
...
template<int radix> inline __device__ void store128_half_Z2Z_ax1( double2 *z, double2 *a, int sx )
{
    #pragma unroll
    for( int i = 0; i < (radix>>1); i++ )
...
template<int radix> inline __device__ void store128_half_Z2Z_ax2( double2 *z, double2 *a, int sx )
{
    #pragma unroll
    for( int i = 0; i < (radix>>1); i++ )
...
template<int radix> inline __device__ void store128_half_Z2Z_ax3( double2 *z, double2 *a, int sx )
{
    #pragma unroll
    for( int i = 0; i < (radix>>1); i++ )
...
inline __device__ void gen_y( int radix, double2 *z, double delta, int tid )
{
    for ( int j = 0; j < radix/2; j++ )
        z[j] = operator_mul_zz( z[j], exp_i( M_PI*(tid + M_base*j)*delta* ( M_points - (tid + M_base*j)) ) );
}

inline __device__ void gen_z( int radix, double2 *z, double delta, int segment, int tid )
{
...
template<int radix> inline __device__ void complexMulExpZ( double2 *y, double2 *z, double dx, double delta, int segment, int tid )
{
    for ( int i = 0; i < radix; i++ )
