...
inline __device__ void load4096_half_C2C_ax1( int radix, float2 *z, float2 *a, int sx )
{
    for( int i = 0; i < radix; i++ )
    	if ( i < (radix>>1) )
    	    z[i] = a[i*sx];
...
template<int radix> inline __device__ void store4096_half_C2C_ax1( float2 *z, float2 *data, int sx, int tid )
{
    #pragma unroll
    for( int i = 0; i < (radix>>1); i++ )  
        data[(i*sx)] = z[rev<radix>(i)];
}
