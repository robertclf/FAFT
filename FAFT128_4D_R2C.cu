...
__global__ void FAFT128_C2C_ax3_axSplit0_dev( float *re, float *im, float dx, float delta, int segment )
{
    int tid = threadIdx.x;
    size_t sector = blockIdx.y*gridDim.x + blockIdx.x;
...
__global__ void FAFT128_C2C_65_ax3_axSplit0_dev( float2 *data65, float dx, float delta, int segment )
{
    int tid = threadIdx.x;
    size_t sector = blockIdx.y*gridDim.x + blockIdx.x;
...
__global__ void FAFT128_C2C_ax3_axSplit1_dev( float *re, float *im, float dx, float delta, int segment )
{
    int tid = threadIdx.x;
    size_t sector = blockIdx.y*gridDim.x + blockIdx.x;
...
__global__ void FAFT128_C2C_65_ax3_axSplit1_dev( float2 *data65, float dx, float delta, int segment )
{
    int tid = threadIdx.x;
    size_t sector = blockIdx.y*gridDim.x + blockIdx.x;
...
__global__ void FAFT128_C2C_ax3_axSplit2_dev( float *re, float *im, float dx, float delta, int segment )
{
    int tid = threadIdx.x;
    size_t sector = blockIdx.y*gridDim.x + blockIdx.x;
...
__global__ void FAFT128_C2C_65_ax3_axSplit2_dev( float2 *data65, float dx, float delta, int segment )
{
    int tid = threadIdx.x;
    size_t sector = blockIdx.y*gridDim.x + blockIdx.x;
...
__global__ void FAFT128_R2C_ax3_dev( float *re, float *im, float2 *data65, float dx, float delta, int segment )
{
    int tid = threadIdx.x;
    size_t sector = blockIdx.y*gridDim.x + blockIdx.x;
    ...
    load128_half_R2C_ax3( 8, y, re, 16 );
			
    GENERAL_FAFT128( y, dx, delta, segment, tid );
    
    store128_half_R2C_ax3<8>( y, re, im, data65, 16, tid );
}

__global__ void FAFT128_C2C_ax0_axSplit3_dev( float *re, float *im, float dx, float delta, int segment )
{
    int tid = threadIdx.x;
    size_t sector = blockIdx.y*gridDim.x + blockIdx.x;
...
__global__ void FAFT128_C2C_65_ax0_axSplit3_dev( float2 *data65, float dx, float delta, int segment )
{
    int tid = threadIdx.x;
    size_t sector = blockIdx.y*gridDim.x + blockIdx.x;
...
__global__ void FAFT128_C2C_ax1_axSplit3_dev( float *re, float *im, float dx, float delta, int segment )
{
    int tid = threadIdx.x;
    size_t sector = blockIdx.y*gridDim.x + blockIdx.x;
...
__global__ void FAFT128_C2C_65_ax1_axSplit3_dev( float2 *data65, float dx, float delta, int segment )
{
    int tid = threadIdx.x;
    size_t sector = blockIdx.y*gridDim.x + blockIdx.x;
...
__global__ void FAFT128_C2C_ax2_axSplit3_dev( float *re, float *im, float dx, float delta, int segment )
{
    int tid = threadIdx.x;
    size_t sector = blockIdx.y*gridDim.x + blockIdx.x;
...
__global__ void FAFT128_C2C_65_ax2_axSplit3_dev( float2 *data65, float dx, float delta, int segment )
{
    int tid = threadIdx.x;
    size_t sector = blockIdx.y*gridDim.x + blockIdx.x;
...
extern "C" int FAFT128_4D_R2C( float *data, float2 *data65, float beta, float delta, int segment, int axes, int makeR2C, int axesSplit )
{
	int success = 1;
	dim3 grid_R2C(64*64, 64);
	...
	switch(axes)
	{
		case 0:
			if (makeR2C == 1){			
				...
				switch(axesSplit)
				{
					...
					case 3:
						FAFT128_C2C_ax0_axSplit3_dev<<< grid_C2C, 16 >>>( data, data, beta, delta, segment );
				                FAFT128_C2C_65_ax0_axSplit3_dev<<< grid_C2C_65, 16 >>>( data65, beta, delta, segment );					
						break;
		...
		case 1:
			if (makeR2C == 1){
				...
				switch(axesSplit)
				{
					...
					case 3:
						FAFT128_C2C_ax1_axSplit3_dev<<< grid_C2C, 16 >>>( data, data, beta, delta, segment );
						FAFT128_C2C_65_ax1_axSplit3_dev<<< grid_C2C_65, 16 >>>( data65, beta, delta, segment );					
						break;
		...
		case 2:
			if (makeR2C == 1){
				...
				switch(axesSplit)
				{
					...
					case 3:
						FAFT128_C2C_ax2_axSplit3_dev<<< grid_C2C, 16 >>>( data, data, beta, delta, segment );
						FAFT128_C2C_65_ax2_axSplit3_dev<<< grid_C2C_65, 16 >>>( data65, beta, delta, segment );											
						break;		
		...
		case 3:
			if (makeR2C == 1){
				FAFT128_R2C_ax3_dev<<< grid_R2C, 16 >>>( data, data, data65, beta, delta, segment );
			}
			else{
				switch(axesSplit)
				{
					case 0:
						FAFT128_C2C_ax3_axSplit0_dev<<< grid_C2C, 16 >>>( data, data, beta, delta, segment );
						FAFT128_C2C_65_ax3_axSplit0_dev<<< grid_C2C_65, 16 >>>( data65, beta, delta, segment );					
						break;
					
					case 1:
						FAFT128_C2C_ax3_axSplit1_dev<<< grid_C2C, 16 >>>( data, data, beta, delta, segment );
						FAFT128_C2C_65_ax3_axSplit1_dev<<< grid_C2C_65, 16 >>>( data65, beta, delta, segment );											
						break;
						
					case 2:
						FAFT128_C2C_ax3_axSplit2_dev<<< grid_C2C, 16 >>>( data, data, beta, delta, segment );
						FAFT128_C2C_65_ax3_axSplit2_dev<<< grid_C2C_65, 16 >>>( data65, beta, delta, segment );											
						break;
